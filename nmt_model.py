...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # self.post_embed_cnn = nn.Conv1d(embed_size, embed_size, 3, padding=1)
        ...
        self.combined_output_projection = nn.Linear(3 * self.hidden_size, self.hidden_size)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # print("source padded shape: ", source_padded.shape)
        x = torch.zeros((len(source_padded), len(source_padded[0]), self.embed_size))
        ...
        # print("packed seq: ", packed_seq.data.shape)
        ...
        # print("enc hiddens: ", enc_hiddens)
        lh = torch.cat((last_hidden[0], last_hidden[1]), 1)
        init_decoder_hidden = self.h_projection(lh)
        lc = torch.cat((last_cell[0], last_hidden[1]), 1)
        init_decoder_cell = self.c_projection(lc)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # print("bmm shapes: ", alpha_t.shape, enc_hiddens.shape)
        # print("alpha_t: ", alpha_t)
        # print("enc_hiddens", enc_hiddens)
        a_t = torch.bmm(alpha_t, enc_hiddens)
        a_t = torch.squeeze(a_t)
        # print("a_t: ", a_t)
        # print("batch size: ", len(Ybar_t[0]), "2* hidden size: ", 2*self.hidden_size)
